...
export function OrderTracking({ orderNumber, total, items, onNewOrder }: OrderTrackingProps) {
  ...
  useEffect(() => {
    if (status === 'completed') {
      confetti({
        ...
        spread: 70,
        origin: { y: 0.6 },
        colors: ['#059669', '#10b981', '#0ea5e9', '#0369a1'],
      });
    }
  }, [status]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
      <div className="bg-emerald-600 text-white p-6 text-center shadow-md">
        <h1 className="text-2xl font-bold mb-2">Seguimiento del Pedido</h1>
      </div>

      <div className="p-6 space-y-6 max-w-2xl mx-auto">
        <div className="bg-white rounded-xl p-8 shadow-lg text-center border-2 border-emerald-100">
          <div className="mx-auto mb-5 inline-flex flex-col items-center rounded-2xl bg-emerald-50 px-6 py-3 text-sky-900 shadow-sm">
            <span className="text-xs font-extrabold uppercase tracking-[0.2em] text-sky-900/70">
              Número de pedido
            </span>
        ...
        <div className="bg-white rounded-xl p-5 shadow-lg border-2 border-emerald-100">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-sm font-bold text-sky-900">
              {progressPercent}%
            </span>
          </div>
                  ...
                  {readyVenues.includes(venueEstimate.key) &&
                    ...
                    !completedVenues.includes(venueEstimate.key) && (
                      <button
                        ...
                        className="mt-3 w-full rounded-lg bg-emerald-600 px-3 py-2 text-sm font-bold text-white transition-colors hover:bg-emerald-700"
                      >
                        Ya llegué
                      </button>
                    )}
              ...
              <div className="mt-5 space-y-3">
                {items.map((item) => (
                  <div
                    ...
                    className="flex items-start justify-between gap-4 rounded-xl border-2 border-emerald-100 bg-slate-50/60 px-4 py-3"
                  >
                    <div className="min-w-0">
                    ...
                    <span className="shrink-0 font-bold text-sky-900">
        ...
        <div className="bg-white rounded-xl p-6 shadow-lg border-2 border-emerald-100">
          <div className="flex justify-between items-center">
            ...
            <span className="text-2xl font-bold text-sky-900">${total.toLocaleString()}</span>
          </div>
        </div>
        ...
        {status === 'ready' && !hasMultipleVenues && venueEstimates[0] && (
          <button
            ...
            className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold hover:bg-emerald-700 transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2 shadow-lg"
          >
        ...
        {status === 'completed' && (
          <button
            onClick={onNewOrder}
            className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold hover:bg-emerald-700 transition-colors shadow-lg"
          >
            Hacer Nuevo Pedido
          </button>
        )}
